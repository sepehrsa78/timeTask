...
class BayesianObserver:
    ...
    def BayesianModel(self, initialValues):
        ...
        if self.fitType == 'BLS':
            f       = self.mrgGauss(d_tm, self.xSvec[:, np.newaxis], initialValues[0], self.xPvec[:, np.newaxis], self.fBLS(d_tm, initialValues[0], initialValues[2]), initialValues[1])
        elif self.fitType == 'MLE':
            f       = self.mrgGauss(d_tm, self.xSvec[:, np.newaxis], initialValues[0], self.xPvec[:, np.newaxis], self.fMLE(d_tm, initialValues[0], initialValues[2]), initialValues[1])
        else:
            f       = self.mrgGauss(d_tm, self.xSvec[:, np.newaxis], initialValues[0], self.xPvec[:, np.newaxis], self.fMAP(d_tm, initialValues[0], initialValues[2]), initialValues[1])
